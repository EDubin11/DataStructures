...
public class zfirstDocumentStoreImplTest {
    ...
    public void stage3DeleteAllWithPrefix() throws IOException {
        ...
        assertEquals(0,results.size(),"search should've returned 0 results");//fails
        URI[] present = new URI[0];
        URI[] absent = {this.uri1,this.uri2,this.uri3,this.uri4};
        this.checkContents(results,present,absent);//this also fails
    ...
    public void testSetTime() throws IOException {
        ...
        store.putDocument(new ByteArrayInputStream(this.txt2.getBytes()),this.uri2, DocumentStore.DocumentFormat.TXT);
        store.putDocument(new ByteArrayInputStream(this.txt4.getBytes()),this.uri4, DocumentStore.DocumentFormat.BINARY);
        Document a = store.getDocument(this.uri1);
        Document b = store.getDocument(this.uri2);
        Document c = store.getDocument(this.uri3);
        Document d = store.getDocument(this.uri4);
        long time1a = a.getLastUseTime();
        store.getDocument(this.uri1);
        long time2a = a.getLastUseTime();
        assertTrue(time1a < time2a);
        System.out.println(time1a);
        System.out.println(time2a);
        List<Document> results = store.search("text");
        //assertTrue(a.equals(c),"failed");
        System.out.println("a code  " + a.hashCode());

        System.out.println("c code  " +c.hashCode());
        long time3a = a.getLastUseTime();
        assertTrue(time2a < time3a);

        int firstToAdd = a.getDocumentTxt().getBytes().length;
        int secondToAdd = b.getDocumentTxt().getBytes().length;
        int thirdToAdd = c.getDocumentBinaryData().length;
        int fourthToAdd = d.getDocumentBinaryData().length;
        System.out.println(firstToAdd + " " + a.getLastUseTime());
        System.out.println(secondToAdd + "  " + b.getLastUseTime());
        System.out.println(thirdToAdd + "  " + c.getLastUseTime());
        System.out.println( fourthToAdd + "  " + d.getLastUseTime());
        System.out.println(firstToAdd + secondToAdd + thirdToAdd + fourthToAdd);
        store.setMaxDocumentBytes(150);
        Document aa = store.getDocument(this.uri1);
        Document bb = store.getDocument(this.uri2);
        Document cc = store.getDocument(this.uri3);
        Document dd = store.getDocument(this.uri4);
